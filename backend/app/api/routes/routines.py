...
async def list_tasks(patient_id: int = 1, user: dict = Depends(get_current_user)):
    ...
    return load_patient_tasks(patient_id, _supabase())
...
async def toggle_task_done(task_id: int, patient_id: int = 1, user: dict = Depends(get_current_user)):
    ...
    if supabase:
        try:
            fetch_res = supabase.table("tasks").select("*").eq("id", task_id).eq("patient_id", patient_id).single().execute()
            if fetch_res.data:
                current_done = fetch_res.data.get("done", False)
                new_done = not current_done
                update_res = supabase.table("tasks").update({"done": new_done}).eq("id", task_id).eq("patient_id", patient_id).execute()
...
async def delete_task(task_id: int, patient_id: int = 1, user: dict = Depends(get_current_user)):
    """
    Deletes a task by ID from Supabase.
    """
    await ensure_patient_access(user, patient_id)
    if not can_write(user):
        raise HTTPException(status_code=403, detail="You cannot delete tasks for this patient.")
    supabase = _supabase()
    if supabase:
        try:
            supabase.table("tasks").delete().eq("id", task_id).eq("patient_id", patient_id).execute()
        except Exception as e:
            logger.error(f"Failed to delete task {task_id} in Supabase: {e}")

    global MOCK_TASKS
    MOCK_TASKS = [t for t in MOCK_TASKS if not (t["id"] == task_id and t["patient_id"] == patient_id)]
    return {"success": True, "message": f"Task {task_id} deleted successfully"}

@router.post("/tasks/reset")
async def reset_tasks(patient_id: int = 1, user: dict = Depends(get_current_user)):
    """
    Resets all tasks for a patient to done = False for the current day.
    """
    await ensure_patient_access(user, patient_id)
    if not can_write(user):
        raise HTTPException(status_code=403, detail="You cannot reset tasks for this patient.")
    supabase = _supabase()
    today = _today_str()
    if supabase:
        try:
...
async def list_reminders(patient_id: int = 1, user: dict = Depends(get_current_user)):
    ...
    await ensure_patient_access(user, patient_id)
    return load_patient_reminders(patient_id, _supabase())
...
async def add_reminder(reminder: ReminderCreate, user: dict = Depends(get_current_user)):
    ...
    if (reminder.frequency or "Daily") not in ALLOWED_FREQUENCIES:
...
async def update_reminder(reminder_id: int, reminder: ReminderUpdate, patient_id: int = 1, user: dict = Depends(get_current_user)):
    ...
    if "frequency" in updates and updates["frequency"] is not None and updates["frequency"] not in ALLOWED_FREQUENCIES:
        raise HTTPException(status_code=400, detail="Invalid reminder frequency.")

    if supabase and updates:
        try:
            current_row = None
            fetch_res = supabase.table("reminders").select("*").eq("id", reminder_id).eq("patient_id", patient_id).single().execute()
            if fetch_res.data:
    ...
    if supabase:
        try:
            ...
            if fetch_res.data:
                ...
                update_res = supabase.table("reminders").update({"done": new_done}).eq("id", reminder_id).eq("patient_id", patient_id).execute()
                if update_res.data and len(update_res.data) > 0:
...
async def toggle_reminder_enabled(reminder_id: int, patient_id: int = 1, user: dict = Depends(get_current_user)):
...
async def delete_reminder(reminder_id: int, patient_id: int = 1, user: dict = Depends(get_current_user)):
    """Deletes a reminder by ID from Supabase."""
    await ensure_patient_access(user, patient_id)
    if not can_write(user):
        raise HTTPException(status_code=403, detail="You cannot delete reminders for this patient.")
    supabase = _supabase()
    if supabase:
        try:
            supabase.table("reminders").delete().eq("id", reminder_id).eq("patient_id", patient_id).execute()
        except Exception as e:
            logger.error(f"Failed to delete reminder {reminder_id} in Supabase: {e}")

    global MOCK_REMINDERS
    MOCK_REMINDERS = [r for r in MOCK_REMINDERS if not (r["id"] == reminder_id and r["patient_id"] == patient_id)]
